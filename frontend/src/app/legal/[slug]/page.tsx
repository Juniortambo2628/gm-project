...
const legalConfig: Record<string, { title: string; description: string; icon: any; status: string; features: string[] }> = {
  "privacy": {
    ...
    features: ["End-to-end encryption", "GDPR compliant", "Data sovereignty"]
  },
  "terms": {
    title: "Terms of service",
    description: "The contractual agreements and parameters guiding the usage of our consultancy platform and services.",
    icon: FileText,
    status: "Revision 4.1",
    features: ["Service SLA", "Usage policy", "Client standards"]
  },
  "legal": {
    ...
    features: ["Data protection", "Privacy focus", "Global standards"]
  },
  "cookies": {
    title: "Cookie policy",
    description: "Details regarding cookie usage, session tokens, and local cache mechanics required for platform stability.",
    icon: Cookie,
    status: "Necessary model",
    features: ["Consent auditing", "Session only", "Privacy priority"]
  }
};
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col pt-32 pb-20">
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-12 relative z-10 w-full mt-10">
        <Link href="/">
             <Button variant="ghost" className="mb-12 h-10 px-0 hover:bg-transparent text-primary text-[13px] font-bold flex items-center gap-2">
        ...
        <div className="animate-fade-in flex flex-col items-start">
            ...
            <h1 className="text-4xl lg:text-5xl text-slate-900 dark:text-white mb-6 leading-tight font-bold">
                {title}
            </h1>
            ...
            <div className="flex flex-wrap gap-4 mb-16">
                {features.map((feature, i) => (
                    <span key={i} className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-5 py-2.5 rounded-full text-[13px] font-bold text-slate-700 dark:text-slate-300">
                        {feature}
                    </span>
                ))}
            </div>
            ...
            <div className="prose dark:prose-invert max-w-none text-slate-600 dark:text-slate-400 text-sm leading-loose">
               ...
               <p className="mb-6 font-bold">
                  If you require immediate access to these agreements or have regulatory concerns, please contact our team directly. All data remains protected under encryption protocols during this transitional phase.
               </p>
            </div>
        </div>
      </div>
