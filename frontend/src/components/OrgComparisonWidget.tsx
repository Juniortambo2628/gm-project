...
const COLORS = ["#470f0b", "#f59e0b", "#3b82f6", "#ec4899", "#8b5cf6"];
...
export function OrgComparisonWidget({ className }: { className?: string }) {
    ...
    return (
      <Card className={cn("col-span-full h-[450px] flex items-center justify-center bg-card shadow-sm border border-border/50", className)}>
        <div className="flex flex-col items-center gap-4 text-muted-foreground">
          ...
          <p className="text-[13px] font-bold">Loading comparisons...</p>
        </div>
      </Card>
    );
  }
  ...
  if (!data || !data.trends.length) {
    return (
      <Card className={cn("col-span-full h-[450px] flex items-center justify-center bg-card shadow-sm border border-border/50", className)}>
        <p className="text-sm font-bold text-muted-foreground">Insufficient benchmark data available.</p>
      </Card>
    );
  }
      ...
      <CardHeader className="border-b border-border/50 bg-muted/10 px-8 py-6 flex flex-row items-center justify-between">
        <div className="space-y-1">
          <CardTitle className="text-lg font-bold text-foreground flex items-center gap-3">
             <IconBlock icon={Globe} />
             Client performance comparisons
          </CardTitle>
          <CardDescription className="text-[13px] text-muted-foreground font-bold ml-12">
            Longitudinal variance mapping across active network entities.
          </CardDescription>
        </div>
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-lg text-xs font-bold">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data.trends} margin={{ top: 20, right: 30, left: 10, bottom: 5 }}>
              ...
              <Legend 
                ...
                height={36} 
                iconType="circle"
                wrapperStyle={{ fontSize: "13px", fontWeight: "700", paddingTop: "0px", paddingBottom: "20px" }}
              />
