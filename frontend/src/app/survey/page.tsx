...
function SurveyContent() {
  ...
  const breadcrumbs = [
    { label: "Participating", path: "/survey" },
  ...
  return (
    <div className="min-h-screen bg-background transition-colors duration-500">
      ...
      <PageHero 
        ...
        badge={poll.organization?.name || "Member Survey"}
        breadcrumbs={breadcrumbs}
      />

      <div className="container mx-auto py-20 px-4 max-w-5xl space-y-12 animate-fade-in relative z-10">
       <div className="text-center space-y-4">
          ...
          <p className="text-sm font-bold text-muted-foreground max-w-2xl mx-auto">{poll.description}</p>
          {isReadOnly && (
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/10 text-amber-600 rounded-full mt-4">
                <span className="text-[13px] font-bold">Read-only view • Response locked</span>
            </div>
          )}
       </div>
       ...
       <div className="space-y-6 pb-6">
             ...
             <Card key={q.id} className="w-full transition-colors hover:border-primary/50">
              <div className="flex flex-col md:flex-row gap-6 p-6 md:p-8">
                <div className="flex-1 space-y-4 pr-0 md:pr-4">
                  ...
                  <CardTitle className="text-lg font-bold md: leading-snug">{q.text}</CardTitle>
                </div>
                ...
                <div className="w-full md:w-5/12 bg-muted/50 rounded-xl p-6 border flex flex-col justify-center">
                    <Slider 
                      ...
                      disabled={isReadOnly}
                    />
                    <div className="flex items-center justify-between mt-6 text-xs font-bold text-muted-foreground">
                        <span>Low</span>
                        <span className="text-primary font-bold bg-background px-3 py-1 rounded-md border shadow-sm">Level {answers[q.id]}</span>
                        <span>High</span>
                    </div>
                </div>
              </div>
        ...
        <div className="flex flex-col items-center gap-4 pt-6 pb-20">
          <p className="text-xs font-bold text-muted-foreground">
            {isReadOnly ? "Thank you for your valuable contribution." : "Review your answers before submitting."}
          </p>
          <Button 
            onClick={handleSubmit} 
            disabled={submitting}
            size="lg"
            className="w-full max-w-md font-bold shadow-lg transition-transform hover:scale-[1.02] active:scale-95"
          >
